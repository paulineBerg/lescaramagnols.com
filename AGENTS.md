...
- les modules autonomes sous `backend/src/PrivateApps/*` sont des sous-modules Git et des paquets Composer de type `caramagnols-private-app`; apres clone, pull ou changement de branche, executer `git submodule sync --recursive` puis `git submodule update --init --recursive` avant Composer ou les tests
- toute modification d'un sous-module prive doit etre commitée et poussée dans son depot dedie avant de mettre a jour le gitlink dans le depot principal; ne jamais laisser un sous-module sale, non initialise ou pointe sur un commit non publie
- tout paquet prive installable doit declarer son autoload PSR-4, ses extensions PHP requises et ses manifests dans `extra.caramagnols-private-app.manifests`; il ne doit embarquer ni point d'entree public autonome, ni secret, ni override local, ni donnee runtime
- un deploiement contenant un changement de gitlink PrivateApps doit passer par `backend/tools/deploy-release.sh`; le deploiement rapide doit refuser ce perimetre pour eviter une synchronisation partielle du module
- tout email par defaut de l'espace prive doit etre configurable depuis l'admin: declarer le sujet et le corps dans les templates mail prives, utiliser des variables explicites de type `{{email}}`, `{{login_url}}`, `{{today}}`, et ne pas ajouter de nouveau texte email hardcode sans template modifiable
- tout controle visuel qui agit comme un bouton doit etre un vrai `<button>`: utiliser `<button type="button">` pour une action JavaScript sans soumission, `<button type="submit">` pour soumettre un formulaire, et ne pas detourner `<a>`, `<summary>`, `<div>` ou `<span>` en bouton sauf lien de navigation reel pour `<a>`
- toute correction de rendu, bouton, interaction JavaScript, formulaire ou comportement admin doit etre testee et validee avant cloture avec un controle adapte au risque: syntaxe PHP, controle statique HTML/JS, test navigateur ou verification manuelle ciblee; ne pas conclure sur une interaction sans validation du chemin utilisateur concerne
- tout ecran admin avec navigation laterale doit garder le menu gauche fixe sur desktop: `nav.admin-nav` reste visible pendant le scroll vertical, le contenu compense sa largeur, et le mode mobile repasse en flux normal sans debordement horizontal
...
- tout haut de page admin ou prive doit rester compact: navigation module, messages, premiere carte, titre et premier formulaire doivent eviter les grands espacements verticaux; sur desktop, privilegier une grille de formulaire courte quand les champs s'y pretent, avec retour lisible en une colonne sur mobile
