...
- lot 8.3 (legacy/docs) :
  ...
  - perimetre de refonte heritage identifie par l'audit hygiene du `2026-04-16` documente dans `docs/README_REFONTE_LOT_C.md` : suppressions suivies `pages/site`, `adminFtyhik5642sZ`, `installsql.php`, `menu_data.php`, `sitemap.php` a livrer a part du nettoyage courant.
  - perimetre de nouveau code produit identifie par ce meme audit documente dans `docs/README_CONSOLIDATION_LOT_D.md` : consolidation par domaines `socle HTTP`, `admin/navigation`, `blog/sql`, `logging`, `outillage frontend`.
