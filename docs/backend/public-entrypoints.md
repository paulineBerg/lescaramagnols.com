...
- leur mapping de remplacement est documente dans `../refonte-lot-c.md`
...
- `/core/blog/log_discussion_client.php`
  ...
  - plafonnement par `IP + endpoint` (limiteur fichier), avec échantillonnage en production (`site.discussions.telemetry_sample_divisor`) et payload de détails borné (`sanitizeClientEventDetails`) pour limiter le bruit d'alerte
- `/rss`
