...
L'audit d'hygiene Git du `2026-04-16` a confirme que les suppressions suivies de `backend/templates/pages/site/**`, des anciens points d'entree admin obfusques, de `backend/public/installsql.php`, de `backend/config/menu_data.php` et de `backend/templates/partials/sitemap.php` relevent d'une refonte structurelle deja engagee, pas d'un nettoyage opportuniste.
Ce perimetre est isole et cartographie dans `docs/refonte-lot-c.md`.
Le meme audit confirme aussi qu'un bloc de vrai code produit doit etre consolide a part : `70` fichiers sous `backend/src`, `57` sous `backend/tests`, `12` templates admin, `6` migrations SQL editoriales et `8` outils frontend. Le decoupage recommande par domaines est documente dans `docs/consolidation-lot-d.md`.
