...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const allMenuItems = document.querySelectorAll<HTMLLIElement>('#menu2 li');
  const hamburger = document.getElementById('hamburger-icon');
  const breadcrumb = document.getElementById('breadcrumb-mobile');
  const mobileNav = breadcrumb ? (breadcrumb.querySelector('.breadcrumb-menu') as HTMLElement | null) : null;
