...
const assetImagesRoot = path.join(projectRoot, 'frontend', 'src', 'assets', 'images');
...
async function main() {
  ...
  if (invalid.length > 0) {
    console.error('[asset-naming] Invalid filenames detected in frontend/src/assets/images.');
  ...
  console.log(`[asset-naming] OK - ${files.length} source files checked.`);
}
