...
const FORBIDDEN_PUBLIC_ENTRYPOINTS = new Set([
  'backend/public/install.php',
...
async function main() {
  ...
  const trackedForbiddenPublicEntrypoints = [...FORBIDDEN_PUBLIC_ENTRYPOINTS].filter((filePath) => {
    return isTracked(filePath);
  });
  if (trackedForbiddenPublicEntrypoints.length > 0) {
    issues.push({
      label: 'Tracked public installer entrypoints',
      items: trackedForbiddenPublicEntrypoints,
    });
  }
