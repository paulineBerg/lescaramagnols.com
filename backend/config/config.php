...
$appConfig = [
    ...
    'identity' => [
        'persistent' => [
            'enabled' => filter_var(env('PERSISTENT_AUTH_ENABLED', false), FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? false,
            'private_enabled' => filter_var(env('PRIVATE_PERSISTENT_AUTH_ENABLED', false), FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? false,
            'admin_enabled' => filter_var(env('ADMIN_PERSISTENT_AUTH_ENABLED', false), FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? false,
            'rotation_enabled' => filter_var(env('PERSISTENT_TOKEN_ROTATION_ENABLED', true), FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? true,
            'reuse_detection_enabled' => filter_var(env('PERSISTENT_TOKEN_REUSE_DETECTION_ENABLED', true), FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? true,
            'revoke_device_on_reuse' => filter_var(env('PERSISTENT_TOKEN_REVOKE_DEVICE_ON_REUSE', true), FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? true,
            'private_cookie_name' => preg_replace('/[^A-Za-z0-9_-]/', '_', trim((string) env('PRIVATE_PERSISTENT_COOKIE_NAME', 'caramagnols_private_persistent'))) ?: 'caramagnols_private_persistent',
            'admin_cookie_name' => preg_replace('/[^A-Za-z0-9_-]/', '_', trim((string) env('ADMIN_PERSISTENT_COOKIE_NAME', 'caramagnols_admin_persistent'))) ?: 'caramagnols_admin_persistent',
            'identity_cookie_name' => preg_replace('/[^A-Za-z0-9_-]/', '_', trim((string) env('IDENTITY_COOKIE_NAME', 'caramagnols_identity'))) ?: 'caramagnols_identity',
