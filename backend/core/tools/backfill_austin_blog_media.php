...
sort($files);
$backfiller = new BlogAustinMediaBackfiller();
$changed = 0;
...
foreach ($files as $file) {
    ...
    $result = $backfiller->backfill($payload);
    if (!$result['changed']) {
        continue;
    }
    $changed++;
    ...
    $payload['updated_at'] = '2026-08-14T12:50:00+02:00';
    $encoded = json_encode(
        $payload,
...
printf("%s: %d variante(s) Austin/Mini complétée(s).\n", $apply ? 'APPLY' : 'DRY-RUN', $changed);
