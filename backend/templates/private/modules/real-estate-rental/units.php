      ...
      <table class="private-click-table">
        ...
        <tbody>
          <?php foreach ($units as $unit) : ?>
                ...
                $unavailableUntil = $unitValue($unit, 'unavailableUntil');
                $statusLabel = (string) ($unitStatuses[$status] ?? $status);
                if ($status === 'unavailable' && $unavailableUntil !== '') {
                    $statusLabel .= ' jusqu’au ' . $unavailableUntil;
                }
        ...
        <dialog class="private-dialog" id="<?php echo htmlspecialchars($dialogId, ENT_QUOTES, 'UTF-8'); ?>" aria-labelledby="<?php echo htmlspecialchars($dialogId . '-title', ENT_QUOTES, 'UTF-8'); ?>">
          <div class="private-dialog-panel">
            ...
            <form method="post" action="<?php echo htmlspecialchars($unitsUrl, ENT_QUOTES, 'UTF-8'); ?>">
              ...
              <label>Indisponible jusqu’au <input type="date" name="unavailable_until" value="<?php echo htmlspecialchars($unitValue($unit, 'unavailableUntil'), ENT_QUOTES, 'UTF-8'); ?>" /></label>
