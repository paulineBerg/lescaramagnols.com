...
<section>
  ...
  <section class="card private-list-section" data-private-filter-scope>
    ...
    <?php else : ?>
      ...
      <table>
        <thead><tr><th>Propriété</th><th>Catégorie</th><th>Nom</th><th>Rattachement</th><th>Poids</th><th>Ajouté le</th><th>Action</th></tr></thead>
        <tbody>
          <?php foreach ($documents as $document) : ?>
                ...
                <?php
                $documentTitle = is_string($document['displayName'] ?? null) && trim((string) $document['displayName']) !== ''
                ...
                ?>
            <tr data-private-filter-row data-filter-text="<?php echo htmlspecialchars(strtolower(trim((string) ($document['propertyName'] ?? '') . ' ' . $category . ' ' . $documentTitle . ' ' . $attachment)), ENT_QUOTES, 'UTF-8'); ?>">
              <td><?php echo htmlspecialchars((string) ($document['propertyName'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></td>
              <td><span class="tag"><?php echo htmlspecialchars($category, ENT_QUOTES, 'UTF-8'); ?></span></td>
              <td><a href="<?php echo htmlspecialchars(rtrim((string) ($urls['documents'] ?? ''), '/') . '/' . rawurlencode($documentId), ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($documentTitle, ENT_QUOTES, 'UTF-8'); ?></a></td>
              <td><?php echo htmlspecialchars($attachment, ENT_QUOTES, 'UTF-8'); ?></td>
    ...
    <div class="private-dialog-panel">
      ...
      <?php else : ?>
        <form method="post" action="<?php echo htmlspecialchars((string) ($urls['documents'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>" enctype="multipart/form-data" data-rental-document-form>
          ...
          <label>Nom du document <input type="text" name="document_name" maxlength="255" placeholder="Vide = nom du fichier" /></label>
