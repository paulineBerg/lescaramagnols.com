...
final class AgencyImportRepository
{
    ...
    public function createAgency(int $createdByPrivateUserId, string $agencyName, array $details = []): bool
    {
        ...
        try {
            ...
            $created = $statement->execute($this->agencyStatementPayload($createdByPrivateUserId, $agencyName, $agencyDetails));
            if ($created) {
                $this->ensureManualAgencyBatch($createdByPrivateUserId, $agencyName, $agencyDetails['notes'] ?? null);
            }
    ...
    private function ensureManualAgencyBatch(int $createdByPrivateUserId, string $agencyName, ?string $notes): void
    {
        try {
            $statement = $this->database->pdo()->prepare(
                sprintf(
